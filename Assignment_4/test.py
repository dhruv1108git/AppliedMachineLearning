...
class TestDocker:
    """Tests for the Docker container"""
    
    def test_docker(self):
        """Test that the Docker container works correctly"""
        # Build the Docker image
        subprocess.run(["sudo", "docker", "build", "-t", "spam-detector", "."], check=True)
        
        # Run the Docker container
        container_process = subprocess.Popen([
            "sudo", "docker", "run", "-d", "-p", "5000:5000", "--name", "spam-detector-container", "spam-detector"
        ])
        ...
        try:
            ...
            assert data["prediction"] == 0  # Should be classified as non-spam
            ...
            subprocess.run(["sudo", "docker", "stop", "spam-detector-container"], check=True)
            subprocess.run(["sudo", "docker", "rm", "spam-detector-container"], check=True)
